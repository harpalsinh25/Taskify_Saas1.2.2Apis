...
public function todos($status = null, $search = '')
{
    ...
    // Apply workspace_id only if present in session
    if (session()->has('workspace_id')) {
        $query->where('workspace_id', session()->get('workspace_id'));
    }
...
{
    return $this->hasMany(Favorite::class); // or morphMany, depending on setup
}
